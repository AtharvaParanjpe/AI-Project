...
class theMaze:
	...
	def bi_directional_BFS(self):
		self.qStart = deque()
		self.qStart.append((0, 0))

		self.qEnd = deque()
		self.qEnd.append((0, 0))
		...
		while(len(self.qStart) > 0 and len(visitedFromLeft)>0):
			self.x, self.y = self.qStart.popleft()
			self.visited.append((self.x, self.y))

			x_end,y_end = self.qEnd.popleft()
			self.visited.append((x_end, y_end))
			...
			self.update_the_maze_simple(self.x, self.y)
			for coord in [(self.x + 1, self.y), (self.x, self.y + 1), (self.x - 1, self.y), (self.x, self.y - 1)]:
				if coord not in self.qStart and coord not in self.visited:
					a, b = coord
					if a >= 0 and b >= 0 and a < self.rows and b < self.columns:
						if self.mapstate[a, b]!=0:
							self.qStart.append(coord)
